...
const TradingSettings = ({
  ...
  const { initialCapital, environment } = usePortfolioConfig(customCapital);
  const [localSettings, setLocalSettings] = useState({
    ...tradingSettings,
    initialCapital: initialCapital, // 🎯 추가
  });
  ...
  useEffect(() => {
    if (
      JSON.stringify(tempSettings) !== JSON.stringify(prevSettingsRef.current)
    ) {
      setHasChanges(true);
    } else {
      setHasChanges(false);
  ...
  const handleAllocationChange = useCallback(
    (key, value) => {
      ...
      const adjustedAllocations = adjustOtherAllocations(
        key,
        numValue,
        currentAllocations
      );
  ...
  const handleCapitalChange = useCallback(
    (newCapital) => {
      const capital = newCapital || PORTFOLIO_CONFIG.INITIAL_CAPITAL;
  ...
  const applyPreset = useCallback(
    (presetName) => {
      ...
      const newSettings = {
        ...
        tradingConditions:
          preset.tradingConditions ||
          getDefaultTradingSettings().tradingConditions,
        strategy: preset.strategy || tempSettings.strategy,
      };

      setTempSettings(newSettings);
      console.log(
        `🔧 ${presetName} 전략 프리셋 적용 (테스트모드: ${testMode}):`,
        newSettings
      );
    },
  ...
  const handleSave = useCallback(() => {
    const normalizedSettings = {
      ...tempSettings,
      portfolioAllocation: normalizeAllocations(
        tempSettings.portfolioAllocation
      ),
    };
    ...
    console.log(
      "💾 [TradingSettings] minBuyScore 값:",
      normalizedSettings.tradingConditions?.buyConditions?.minBuyScore
    );
    ...
    try {
      if (typeof window !== "undefined" && window.localStorage) {
        const savedConfig = {
          ...
          buyConditions: {
            minScore:
              normalizedSettings.tradingConditions?.buyConditions
                ?.minBuyScore ?? null,
            rsiThreshold:
              normalizedSettings.tradingConditions?.buyConditions
                ?.rsiOversold ?? null,
            confidenceLevel: normalizedSettings.tradingConditions?.buyConditions
              ?.strongBuyScore
              ? normalizedSettings.tradingConditions.buyConditions
                  .strongBuyScore / 10
              : 0,
          ...
          sellConditions: {
            profitTarget:
              normalizedSettings.tradingConditions?.sellConditions
                ?.profitTarget1 ?? null,
            stopLoss:
              normalizedSettings.tradingConditions?.sellConditions?.stopLoss ??
              null,
            rsiOverbought:
              normalizedSettings.tradingConditions?.sellConditions
                ?.rsiOverbought ?? null,
        ...
        window.localStorage.setItem(
          "cryptowise_trading_config",
          JSON.stringify(savedConfig)
        );
        console.log(
          "💾 로컬 저장소에 저장됨: cryptowise_trading_config",
          savedConfig
        );
      }
    } catch (error) {
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border">
            <div className="flex items-center space-x-2 mb-3">
              ...
              <h3 className="font-semibold text-gray-800">초기 자본 설정</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  value={customCapital || ""}
                  onChange={(e) =>
                    handleCapitalChange(Number(e.target.value) || null)
                  }
                  placeholder={`기본값: ${PORTFOLIO_CONFIG.INITIAL_CAPITAL.toLocaleString()}원`}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  disabled={isActive}
                />
                <p className="text-xs text-gray-500 mt-1">
                  현재 환경: {environment} | 적용될 금액:{" "}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      key={value}
                      onClick={() => handleCapitalChange(value)}
                      disabled={isActive}
            ...
            {isActive && (
              <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-700">
                💡 거래 활성 중에는 초기 자본을 변경할 수 없습니다. 거래를
                중지한 후 변경해주세요.
              </div>
            )}
          </div>
                    ...
                    : presetName === "conservative"
                      ...
                      : presetName === "balanced"
                        ...
                        : presetName === "aggressive"
                          ...
                          : presetName;
                const minScore =
                  preset?.tradingConditions?.buyConditions?.minBuyScore ?? "-";
                return (
                  <button
                    key={presetName}
              ...
              <div>
                ...
                <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  ...
                  <div className="text-sm">
                    <div
                      className={`font-bold ${portfolioPreview.isValid ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                    >
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                      평균 대비 거래량 배수 (
                      {riskManagement.volumeThreshold || 1.5}배)
                    </label>
                    <input
                      type="range"
                      min="1.0"
