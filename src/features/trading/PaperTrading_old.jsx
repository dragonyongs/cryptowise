...
import { useTradingStore } from "../../stores/tradingStore"; // 거래 설정 스토어
...
const PaperTrading = ({ userId = "demo-user", externalSettings = null }) => {
  ...
  // 🔥 중앙화된 설정들 가져오기
  const portfolioConfig = usePortfolioStore((state) => state.config);
  const tradingSettings = useTradingStore((state) => state.tradingSettings);

  // 🔥 실시간 거래 설정 가져오기
  const engineSettings = useMemo(() => {
    if (window.paperTradingEngine) {
      return window.paperTradingEngine.getCurrentSettings();
    }
    return null;
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('engine-settings-changed', handleConfigUpdate);

    return () => {
      window.removeEventListener('trading-config-updated', handleConfigUpdate);
      window.removeEventListener('engine-settings-changed', handleConfigUpdate);
    };
  }, []);
  ...
  const summary = useMemo(() => {
    ...
    // 실제 설정들에서 값 추출
    const actualMinBuyScore = tradingSettings?.minBuyScore ||
      engineSettings?.tradingLimits?.minSignalScore ||
      8.0;

    const actualProfitTarget = tradingSettings?.profitTarget ||
      engineSettings?.tradingLimits?.profitTarget ||
      8;

    const actualStopLoss = tradingSettings?.stopLoss ||
      engineSettings?.tradingLimits?.stopLoss ||
      -6;

    const actualMaxPositions = engineSettings?.tradingLimits?.maxPositions ||
      tradingSettings?.maxPositions ||
      4;

    // 실제 tier 배분 가져오기
    const tierAllocation = engineSettings?.tierAllocation || {
      TIER1: 0.55,
      TIER2: 0.30,
      TIER3: 0.15
    };
    ...
    // 현금 비중 계산
    const cashRatio = engineSettings?.tradingLimits?.reserveCashRatio || 0.4;

    // 전략 레이블 동적 계산
    let strategyLabel = "균형";
    if (actualMinBuyScore >= 8.5) {
      strategyLabel = "초보수적";
    } else if (actualMinBuyScore >= 7.5) {
      strategyLabel = "보수적";
    } else if (actualMinBuyScore >= 6.5) {
      strategyLabel = "균형";
    } else {
      strategyLabel = "적극적";
    }
    ...
    return {
      strategyLabel,
      minBuyScore: actualMinBuyScore.toFixed(1),
      profitTarget1: actualProfitTarget,
      stopLoss: actualStopLoss,
      maxCoinsToTrade: actualMaxPositions,
      allocation: {
        cash: Math.round(cashRatio * 100),
        t1: Math.round(tierAllocation.TIER1 * 100),
        t2: Math.round(tierAllocation.TIER2 * 100),
        t3: Math.round(tierAllocation.TIER3 * 100)
      },
      testMode: engineSettings?.isTestMode || false,
      totalRules: Object.keys(tradingSettings || {}).length
    };
  }, [portfolioConfig, tradingSettings, engineSettings]);
