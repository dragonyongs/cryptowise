...
export default function CoinAnalysis() {
    ...
    const { selectedCoins = [] } = JSON.parse(localStorage.getItem('cryptowise-watchlist') || '[]');
        ...
        BTC: {
            ...
            analysis: {
                overall: {
                    ...
                    confidence: 'HIGH',
                    reasoning: '강력한 기술적 지표와 긍정적인 시장 심리가 결합된 매수 신호'
                },
                technical: {
                    score: 8.2,
                    indicators: {
                ...
                sentiment: {
                    score: 7.8,
                    news: { score: 8.2, trend: 'positive', headlines: 3 },
                    social: { score: 7.5, mentions: 15420, sentiment: 'bullish' },
                    institutional: { score: 7.6, flow: 'inflow', amount: '234M USD' }
                }
            },
            signals: [
                {
                ...
                },
                {
                    ...
                    strength: 'medium',
                    indicator: 'Support',
                    description: '주요 지지선 63.5만원 근처에서 반등',
                    timestamp: '25분 전'
                }
            ]
        }
    ...
    const coins = [
        { symbol: 'BTC', name: '비트코인', score: 8.5, change: 2.3 },
        { symbol: 'ETH', name: '이더리움', score: 8.1, change: -0.8 },
        { symbol: 'ADA', name: '에이다', score: 7.2, change: 4.2 },
        { symbol: 'DOT', name: '폴카닷', score: 6.8, change: -1.5 },
        { symbol: 'LINK', name: '체인링크', score: 7.6, change: 3.1 }
    ];
    ...
    useEffect(() => {
        ...
        // 분석 데이터 로딩 시뮬레이션
        setTimeout(() => {
            setAnalysisData(mockAnalysisData[selectedCoin] || mockAnalysisData.BTC);
            setLoading(false);
        }, 1500);
    ...
    const getSignalColor = (recommendation) => {
        switch (recommendation) {
            ...
            case 'BUY': return 'text-blue-600 bg-blue-50 border-blue-200';
    ...
    const getSignalText = (recommendation) => {
        switch (recommendation) {
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-xl shadow-sm border border-crypto-neutral-200 p-4">
                            <div className="flex items-center space-x-2 mb-4">
                                ...
                                <h2 className="font-semibold text-crypto-neutral-900">코인 목록</h2>
                            </div>
                            ...
                            <div className="space-y-2">
                                {coins.map((coin) => (
                                    <motion.button
                                        ...
                                        className={`w-full p-3 rounded-lg text-left transition-colors ${selectedCoin === coin.symbol
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        ...
                        >
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className={`flex items-center text-sm ${analysisData?.change24h >= 0 ? 'text-green-600' : 'text-red-600'
                                                }`}>
                                ...
                                <div className="text-right">
                                    <div className={`inline-flex items-center px-4 py-2 rounded-full border ${getSignalColor(analysisData?.analysis?.overall?.recommendation)
                                        }`}>
                                        <BoltIcon className="w-4 h-4 mr-2" />
                                        <span className="font-semibold">
                                            {getSignalText(analysisData?.analysis?.overall?.recommendation)}
                                        </span>
                                    </div>
                                    <div className="text-sm text-crypto-neutral-600 mt-1">
                                        신뢰도: {analysisData?.analysis?.overall?.confidence}
                                    </div>
                                </div>
                            </div>

                            <div className="bg-crypto-neutral-50 rounded-lg p-4 mb-6">
                                <h3 className="font-medium text-crypto-neutral-900 mb-2">AI 분석 결과</h3>
                                <p className="text-crypto-neutral-700 text-sm leading-relaxed">
                                    {analysisData?.analysis?.overall?.reasoning}
                                </p>
                            </div>
                        ...
                        >
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                {Object.entries(analysisData?.analysis?.technical?.indicators || {}).map(([key, indicator]) => (
                            ...
                            <div className="mt-6 grid md:grid-cols-2 gap-4">
                                <div className="p-4 bg-green-50 rounded-lg">
                                    ...
                                    <div className="text-xl font-bold text-green-600">
                                        ₩{analysisData?.analysis?.technical?.support?.toLocaleString()}
                                    </div>
                                </div>
                                <div className="p-4 bg-red-50 rounded-lg">
                                    <h4 className="font-medium text-red-900 mb-2">저항선</h4>
                                    <div className="text-xl font-bold text-red-600">
                                        ₩{analysisData?.analysis?.technical?.resistance?.toLocaleString()}
                                    </div>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            ...
                            <button
                                onClick={() => navigate('/trading', { state: { selectedCoins: [selectedCoin], analysisData } })}
