// src/components/features/testing/TradingSettings.jsx - 기존 UI + 개선된 거래 조건 통합
...
const TradingSettings = ({
  ...
  const getDefaultTradingSettings = useCallback(() => ({
    ...
    tradingConditions: {
      ...
      buyConditions: {
        ...
        buyThreshold: testMode ? -1.5 : -2.0, // 매수 기준점 (가격 하락률)
        requireMultipleSignals: true,
      },

      // 매도 조건  
      sellConditions: {
        profitTarget1: 3,          // 1차 수익실현 3%
        profitTarget2: 5,          // 2차 수익실현 5%  
        profitTarget3: 8,          // 3차 수익실현 8%
        stopLoss: -6,              // 손절 -6%
        sellThreshold: testMode ? 2.0 : 3.0, // 매도 기준점 (가격 상승률)
        rsiOverbought: testMode ? 65 : 70,
        timeBasedExit: 7,          // 7일 후 자동 매도
      },
  ...
  const presetStrategies = useMemo(() => ({
    ultraConservative: {
      portfolioAllocation: { cash: 0.5, t1: 0.4, t2: 0.075, t3: 0.025 },
      tradingConditions: {
        buyConditions: {
          minBuyScore: testMode ? 7.5 : 9.0,
          buyThreshold: testMode ? -2.5 : -3.0,
          rsiOversold: testMode ? 32 : 28,
          strongBuyScore: testMode ? 8.5 : 9.5,
        },
        sellConditions: {
          profitTarget1: 2,
          profitTarget2: 4,
          profitTarget3: 6,
          stopLoss: -4,
          sellThreshold: testMode ? 1.5 : 2.0,
        },
        riskManagement: {
          maxCoinsToTrade: testMode ? 3 : 2,
          dailyTradeLimit: testMode ? 8 : 4,
        }
      },
    ...
    balanced: {
      portfolioAllocation: { cash: 0.3, t1: 0.385, t2: 0.21, t3: 0.105 },
      tradingConditions: {
        buyConditions: {
          minBuyScore: testMode ? 6.0 : 8.0,
          buyThreshold: testMode ? -1.5 : -2.0,
          rsiOversold: testMode ? 38 : 33,
          strongBuyScore: testMode ? 7.5 : 8.5,
        },
        sellConditions: {
          profitTarget1: 4,
          profitTarget2: 6,
          profitTarget3: 10,
          stopLoss: -7,
          sellThreshold: testMode ? 3.0 : 4.0,
        },
        riskManagement: {
          maxCoinsToTrade: testMode ? 5 : 4,
          dailyTradeLimit: testMode ? 12 : 8,
        }
      },
      strategy: "balanced"
    },
    ...
    aggressive: {
      portfolioAllocation: { cash: 0.25, t1: 0.3, t2: 0.2625, t3: 0.1875 },
      tradingConditions: {
        buyConditions: {
          minBuyScore: testMode ? 5.5 : 7.5,
          buyThreshold: testMode ? -1.0 : -1.5,
          rsiOversold: testMode ? 40 : 35,
          strongBuyScore: testMode ? 7.0 : 8.0,
        },
        sellConditions: {
          profitTarget1: 5,
          profitTarget2: 8,
          profitTarget3: 12,
          stopLoss: -8,
          sellThreshold: testMode ? 4.0 : 5.0,
        },
        riskManagement: {
          maxCoinsToTrade: testMode ? 6 : 5,
          dailyTradeLimit: testMode ? 15 : 10,
        }
      },
      strategy: "aggressive"
    }
  ...
  const applyPreset = useCallback((presetName) => {
    ...
    const normalizedAllocation = normalizeAllocations(preset.portfolioAllocation);

    const newSettings = {
      ...tempSettings,
      portfolioAllocation: normalizedAllocation,
      tradingConditions: preset.tradingConditions,
      strategy: preset.strategy,
    };

    setTempSettings(newSettings);
    console.log(`🔧 ${presetName} 전략 프리셋 적용 (테스트모드: ${testMode}):`, newSettings);
  }, [presetStrategies, tempSettings, testMode]);
  ...
  const sections = [
    ...
    { id: 'trading', name: '거래 조건', icon: ZapIcon }, // 🎯 NEW
              ...
              {Object.keys(presetStrategies).map((presetName) => (
                ...
                >
                  <div className="text-sm font-medium text-gray-900 dark:text-white capitalize mb-2">
                    {presetName === 'ultraConservative' ? '초보수적' :
                      presetName === 'conservative' ? '보수적' :
                        presetName === 'balanced' ? '균형' :
                          presetName === 'aggressive' ? '적극적' : presetName}
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    {presetStrategies[presetName].tradingConditions.buyConditions.minBuyScore}점 이상
                  </div>
                </button>
              ))}
            </div>
